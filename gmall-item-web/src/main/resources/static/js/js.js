$(function(){
    ...
    function switchSkuId() {
        ...
        var valueIdkeysArray=[];
        for (i = 0; i < redborderDivs.length; i++) {
            var redborderDiv = redborderDivs.eq(i);
            var attrValueId = redborderDiv.attr("value");
            valueIdkeysArray.push(attrValueId);
        }
        valueIdkeysArray.sort(function (m, n) {
           return m-n;
        });
        var valueIdkeys = "";

        for (i = 0; i < valueIdkeysArray.length; i++) {
            var attrValueId = valueIdkeysArray[i];
            if (i > 0) {
                valueIdkeys += "|";
            }
